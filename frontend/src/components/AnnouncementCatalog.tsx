...
export default async function AnnouncementCatalog({
  ...
  const announcementJsonFiltered =
    userRole !== "admin"
      ? announcementJsonReady?.data.filter(
      ...
      console.log("End: ", endDate.getTime()); */
            const afterStartRange =
              Date.now() - (Date.now() % 86400000) >= startDate.getTime(); // StartDate is greater than midnight of today (calculated by: Now in millisecond - millisecond from today midnight till now)
            /*       console.log("afterStartRange: ", afterStartRange); */
            const beforeEndRange =
              !isNaN(new Date(endDate).getTime()) &&
              new Date(endDate).getTime() !== 0
                ? Date.now() + (86400000 - (Date.now() % 86400000)) <=
                  endDate.getTime()
                : true; // EndDate is lower than end of today (calculated by: Now in millisecond + millisecond now till end of day)
    ...
    <main>
      {announcementJsonFiltered?.map((announcementItem: AnnouncementItem) => (
        <AnnouncementCard
          ...
          endDate={
            announcementItem.endDate ? new Date(announcementItem.endDate) : null
          }
