  ...
  return (
    <div>
      {transaction === null ? (
        ...
        <div
          id="transaction-card"
          className="flex flex-row text-center items-center justify-center my-5"
        >
          <div className="w-1/5">
            <div>{user}</div>
          </div>
          ...
          {transaction.status === "REJECTED" && role !== "admin" ? (
            <Link
              href={`/payment/edit?tid=${transaction._id}`}
              className="w-1/5"
            >
            ...
            <Link href={`/transaction/${transaction._id}`} className="w-1/5">
              <button
                className="justify-center bg-white border-[2px] border-fern px-6 py-1 text-fern font-medium rounded-full hover:shadow-lg"
