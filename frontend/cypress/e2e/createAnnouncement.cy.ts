/*describe('Test create announcement function by user role', () => {
    beforeEach(() => {
        ...
        cy.get('input[name="email"]').type('user123@gmail.com').should('have.value', 'user123@gmail.com')
        cy.get('input[name=password]').type('user1234').should('have.value', 'user1234')
...
describe('Test create announcement function by admin role', () => {
    beforeEach(() => {
        ...
        cy.get('input[name=password]').type('admin1234').should('have.value', 'admin1234')
        cy.get('button').contains('Login').click()
        cy.url().should('eq', 'http://localhost:3000/')
        cy.get('button').contains('SIGN OUT').should('contain', 'SIGN OUT')
        cy.get('button').contains('CAMPGROUND').click()
        ...
        cy.get('button[id="createAnncButton"]').should('exist').click({ force: true })
