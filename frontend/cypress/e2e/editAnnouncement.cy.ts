describe("Test create transaction function", () => {
  ...
  it("1. Check title cannot be empty", () => {
    ...
    cy.get(
      ":nth-child(2) > .bg-white > .justify-between > .flex > :nth-child(1)"
    ).click();
    cy.wait(1000);
    cy.get(
      ".text-sm.max-w-lg.min-w-sm.min-h-14.w-full.border.rounded-md.p-2.bg-gray-100.border-1.border-cadetblue.mt-4"
    )
      .eq(0)
      .should("exist");
    cy.get(
  ...
  it("2. Check content cannot be empty", () => {
    ...
    cy.wait(1000);
    cy.get(
      ".text-sm.max-w-lg.min-w-sm.min-h-14.w-full.border.rounded-md.p-2.bg-gray-100.border-1.border-cadetblue.mt-4.mb-4",
      { timeout: 10000 }
    )
      .eq(0)
      .should("exist");
